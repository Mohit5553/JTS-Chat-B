  ...
  io.on("connection", async (socket) => {
    ...
    socket.on("agent:join-session", ({ sessionId }) => {
      ...
      for (const room of socket.rooms) {
        ...
        logger.log(`[SOCKET_LEAVE]: Agent leaving session room ${room}`);
        socket.leave(room);
      }

      logger.log(`[SOCKET_JOIN]: Agent ${socket.data.user?._id} joining room ${sessionId}`);
      socket.join(sessionId);
    });

    socket.on("visitor:join-room", ({ sessionId }) => {
      logger.log(`[SOCKET_JOIN]: Visitor joining room ${sessionId}`);
      socket.join(sessionId);
    });
    ...
    socket.on("visitor:message", async ({ sessionId, message, attachmentUrl = null, attachmentType = null, tempId = null }) => {
      try {
        ...
        logger.log(`[VISITOR_STEP 1]: Event received for session ${sessionId}`);
        ...
        logger.log(`[VISITOR_STEP 2]: Session found: ${session._id}`);
        ...
        if (session.status === "closed") {
          ...
          logger.log(`[VISITOR_STEP 2b]: Session re-opened and saved for ${session.sessionId}`);
        }
        ...
        if (!session.assignedAgent) {
          ...
          if (agent && !await hasReachedActiveChatLimit(agent._id)) {
            ...
            logger.log(`[VISITOR_STEP 2a]: Auto-assigned to agent ${agent._id}`);
        ...
        logger.log(`[VISITOR_STEP 3]: Persistence started for message`);
        ...
        logger.log(`[VISITOR_STEP 4]: Saved to DB: ${saved?._id}`);
        const payload = {
          _id: saved._id,
          sessionId: session.sessionId,
        ...
        logger.log(`[VISITOR_STEP 5]: Broadcast complete for session ${session.sessionId}`);
      } catch (err) {
    ...
    socket.on("agent:message", async ({ sessionId, message, attachmentUrl = null, attachmentType = null, tempId = null }) => {
      try {
        ...
        logger.log(`[AGENT_STEP 1]: Event received for session ${sessionId} by ${user.name}`);
        const session = await ChatSession.findOne({ sessionId })
          .populate("websiteId")
        ...
        logger.log(`[AGENT_STEP 2]: Session found: ${session._id}`);
        ...
        if (session.status === "closed") {
          ...
          session.closedAt = null;
          wasClosed = true;
          logger.log(`[AGENT_STEP 2b]: Session re-opened by agent message for ${session.sessionId}`);
        }
        ...
        logger.log(`[AGENT_STEP 3]: Persistence started for message`);
        const saved = await addMessage({
          chatSession: session,
          sender: "agent",
          message: message || "",
        ...
        logger.log(`[AGENT_STEP 4]: Saved to DB: ${saved?._id}`);
        const payload = {
          _id: saved._id,
          sessionId: session.sessionId,
          message: saved.message,
        ...
        logger.log(`[AGENT_TRACE]: Broadcasting to ${targetRoom}. Clients in room: ${roomClients ? roomClients.size : 0}`);
        ...
        logger.log(`[AGENT_STEP 5]: Broadcast complete for session ${targetRoom}`);
      } catch (err) {
